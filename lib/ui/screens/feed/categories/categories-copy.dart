          ...
          Expanded(
            child: StreamBuilder(
              ...
              builder: (context, category) => !category.hasData
                  ...
                  : _seriesFilter==1?FutureBuilder(
                      key: _futureKey,
                      future:VideosAPI.getVideosByCategory(category.data,_currentPage,_seriesFilter),
                      builder:(context, videos) => videos.connectionState !=
                        ...
                        key: _futureKey,
                        future:VideosAPI.getVideosByPlaylist(_seriesFilter),
                         builder:(context, videos) => videos.connectionState !=
                         ConnectionState.done ||videos.hasError || videos.hasData && videos.data.error != false || videos.hasData &&videos.data.response.videos.isEmpty
                         ? SizedBox(
                              width: MediaQuery.of(context).size.width,
                              height: MediaQuery.of(context).size.height -kToolbarHeight,
                               child: videos.hasData &&videos.data.response.videos.isEmpty
                                ? Center(
                                      child: Text(
                                            'No videos found in this category',
                               ...
                               : ListView(controller: _scrollController,
                                  ...
                                  children: [
                                     GridView.builder(
                                       ...
                                       physics: const NeverScrollableScrollPhysics(),
                                       gridDelegate:
                                      SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 10,childAspectRatio: 16 / 10,
                                                                              mainAxisSpacing: 10,
                                                                ),
                                        ...
                                        padding: const EdgeInsets.symmetric(
                                         ...
                                         child: Row(
                                             ...
                                             children: [
                                               ...
                                               if (videos.data.response.videos.length > 60 && _currentPage != 1)
                                                  const SizedBox(width: 16),
                                               if (videos.data.response.videos.length > 6)
                                                    ...
                                                    focusNode: _goToTopButtonFocusNode,
                                                     ...
                                                     _scrollTo(0);
                                                   FocusScope.of(context).requestFocus(
                                                     NavigationBar.state.categoriesButtonFocusNode);
                                                         },
                                                       ),
                                                 if (videos.data.response.videos.length ==60)
                                                     const SizedBox(width: 16),
                                                   if (videos.data.response.videos.length ==60)
                                                   FocusableButton(focusNode: _nextPageButtonFocusNode,
                                                     width: MediaQuery.of(context).size.width /5,
                                                      height: 36,
                                                     label: 'NEXT PAGE',
                                                     inverted: true,
