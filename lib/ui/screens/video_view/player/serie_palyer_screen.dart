...
import 'package:americonictv_tv/logic/api/models/series_model.dart';
...
import 'package:wakelock/wakelock.dart';
        ...
        class SeriePlayerScreen extends StatefulWidget {
          ...
          State<StatefulWidget> createState() {
            return _SeriePlayerScreenState();
          }
        }

        class _SeriePlayerScreenState extends State<SeriePlayerScreen> {
          ...
          final _reverseButtonKey = GlobalKey<FFRButtonState>();
          ...
          Widget build(BuildContext context) {
            return WillPopScope(
              child: RawKeyboardListener(
                              ...
                              child: DecoratedBox(
                                ...
                                child: Stack(
                                  children: [
                                    ...
                                    Padding(
                                      padding:
                                      const EdgeInsets.fromLTRB(160, 8, 160, 44),
                                      child: Row(
                                        mainAxisSize: MainAxisSize.min,
                                        children: [
                                          StopButton(
                ...
                onKey: (key) {
                  bool _code(int code) =>
                      key.data.toString().contains('keyCode: $code');
                  if (key.runtimeType == RawKeyDownEvent) {
                    final bool _leftKey = _code(Buttons.left.code);
                    final bool _rightKey = _code(Buttons.right.code);
                    ...
                    if (_code(Buttons.ffr.code))
                      _reverseButtonKey.currentState.action();
                    else if (_code(Buttons.ffw.code))
                      _ffwButtonKey.currentState.action();
                    else if (_code(Buttons.playPause.code))
                      _playPauseButtonKey.currentState.action();
                    if (_buttonsVisible) {
                      _visibilityTimer.cancel();
                      _setVisibilityTimer();
                    ...
                    if (_sliderFocusNode.hasFocus && _code(Buttons.up.code))
                      _playPauseButtonFocusNode.requestFocus();
                  }
                },
              ),
